...
Unknown top-level keys are ignored by the graph loader so one rule file
can carry downstream view metadata. The old `collapse:` and `rename:`
keys are not used by `tx-graph`; downstream tools such as
`cardano-tx-tools` may consume them after depending on this repository.
