...
A "scoop" here means a transaction that consumes at least one UTxO
sitting at the Sundae V3 order script (payment-credential hash
`fa6a58bb…`, the same script the case study refers to as
`sundae.swap.v3.order`). This query restricts the candidate seeds
to the May 2026 seed batch via the inline `VALUES` block so the
result matches the case study's "two scoops in May" claim.
...
Two seed transactions in May consume Sundae order UTxOs: the
**9-order scoop dive** `4e2642…` (the headline the operator asked
to demo) and the 1-order swap-cancel `a8bab7bf…` that pulls one
order back out of the pool. The query doesn't pattern-match on tx
shape — it pattern-matches on *consumption of a
`sundae.swap.v3.order` UTxO* via the closure JOIN.
