...
(Total = 30 — matches the seed batch size.)

The single 4-required-signer transaction is the contingency disburse
`18d57a4f…` — the highest-authority shape in the batch. The
six 1-required-signer transactions are the five 11-input reorganizes
(`71ff129b`, `021e6b48`, `65bfd939`, `cb8f1a12`, `019586ee`) plus the
nine-order scoop dive `4e2642…`. The remaining 23 two-required-signer
transactions cover the other two disburses, the 20 swap-order opens,
and the swap cancel `a8bab7bf…` (verified per-seed in
[selections.txt](../selections.txt) crossed with the per-seed signer
count). The fact that the reorganizes and the scoop are 1-signer
contradicts the case study's earlier guess that 1-signer covered
"swap-order opens, swap-cancel and scoop participation" — the actual
shape mapping is read off the chain via the query above.
