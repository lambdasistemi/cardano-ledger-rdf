...
| seed tx count | total fee (lovelace) | min fee (lovelace) | max fee (lovelace) |
|---------------|---------------------:|-------------------:|-------------------:|
| 30            | 19,931,398           | 244,261            | 1,572,508          |

In ADA: total **19.931398 ADA**, min **0.244261 ADA**, max **1.572508 ADA**.

The most expensive single transaction in the seed batch is **not** the
contingency disburse: it is `71ff129b…` at 1.572508 ADA, a reorganize
that consumes **11 inputs** at the network_compliance scope. The next
four most expensive seeds are also 11-input reorganizes
(`021e6b48`, `65bfd939`, `cb8f1a12`, `019586ee`), all between 1.569 and
1.571 ADA. The contingency disburse `18d57a4f` only consumes 2 inputs
and pays **0.415814 ADA** in fee. The causal driver of the
per-transaction fee in this batch is input count (and the script and
redeemer execution units each input drags along), not multisig shape.
