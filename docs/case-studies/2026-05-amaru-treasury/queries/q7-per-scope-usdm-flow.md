...
WHERE {
  {
    ...
    ?asset cardano:hasIdentifier ?id ; cardano:quantity ?qIn .
    ?id cardano:bytesHex
  ...
  UNION
  {
    ...
    ?parentOut cardano:hasIndex ?ix ;
               ...
               cardano:hasAssetValue/rdf:rest*/rdf:first ?asset .
    ?asset cardano:hasIdentifier ?id ; cardano:quantity ?qOut .
    ?id cardano:bytesHex
          "c48cbb3d5e57ed56e276bc45f99ab39abe94e6cd7ac39fb402da47ad0014df105553444d" .
    BIND (0 AS ?qIn)
  }
...
| scope                                     | USDM in (raw) | USDM out (raw) | net (raw)     | net (USDM)      |
|-------------------------------------------|--------------:|---------------:|--------------:|----------------:|
| amaru-treasury.network_compliance         | 1,146,156,659,602 | 1,554,849,981,833 | −408,693,322,231 |    **−408,693.322231** |
| amaru.cag-payee                           |   418,750,000,000 |             0 |   418,750,000,000 |     **+418,750.000000** |
| other (SundaeSwap pool, batchers)         |   490,819,149,109 |   500,875,826,878 |   −10,056,677,769 |      **−10,056.677769** |

(USDM has 6 decimals; divide raw by 10⁶.)

TOTAL in − out (raw) = (1,146,156,659,602 + 418,750,000,000 +
490,819,149,109) − (1,554,849,981,833 + 0 + 500,875,826,878)
= 2,055,725,808,711 − 2,055,725,808,711 = **0** — conservation
exact within the USDM dimension of the seed batch.

network_compliance lost 408,693.322231 USDM net over the seed batch —
418,750 of that went to cag-payee (vendor bridge); the
10,056.677769 USDM "shortfall" was absorbed by SundaeSwap batcher
fees / slippage (visible directly in the `other` row).
