...
commit_gate() {
  ...
  for sha in $(git log --format=%H "$range" 2>/dev/null); do
    ...
    if [ "$(git log -1 --format=%P "$sha" | wc -w)" -eq 1 ]; then
      if ! echo "$body" | grep -qE '^Tasks: (drep-rerun|T005-S1|T006-S1|T007-S1|T013-S1|T014-S1|T017-S1|T022-(S[1-2]|F)|T026-(S1|F)|T028-S1|T030-S1|T031-S1|T040-S1)(, (drep-rerun|T005-S1|T006-S1|T007-S1|T013-S1|T014-S1|T017-S1|T022-(S[1-2]|F)|T026-(S1|F)|T028-S1|T030-S1|T031-S1|T040-S1))*$'; then
