...
commit_gate() {
  ...
  for sha in $(git log --format=%H "$range" 2>/dev/null); do
    ...
    if [ "$(git log -1 --format=%P "$sha" | wc -w)" -eq 1 ]; then
      if ! echo "$body" | grep -qE '^Tasks: (T009-S1|T010-S1|T011-S1|T012-S1|T013-S1|T014-S1|T017-S1|T022-(S[1-2]|F)|T026-(S1|F)|T028-S1|T030-S1|T031-S1|T040-S1)(, (T009-S1|T010-S1|T011-S1|T012-S1|T013-S1|T014-S1|T017-S1|T022-(S[1-2]|F)|T026-(S1|F)|T028-S1|T030-S1|T031-S1|T040-S1))*$'; then
