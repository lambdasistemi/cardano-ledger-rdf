{- |
Module      : Fixtures.TxGraph.S05_WithdrawalScriptStake
...
distinction is exercised by the rules-overlay emit path against
...
level; that's the tx-graph emitter's job).
-}
module Fixtures.TxGraph.S05_WithdrawalScriptStake (
    storyId,
    tx,
