{- |
Module      : Fixtures.TxGraph.S10_GovernanceTreasuryWithdrawal
...
the single withdrawal target to @cardano-foundation.ops@ via the tx-graph
...
map, anchor) — those rendering distinctions are exercised by the
rules-overlay emit path against @rules.yaml@, not here.
