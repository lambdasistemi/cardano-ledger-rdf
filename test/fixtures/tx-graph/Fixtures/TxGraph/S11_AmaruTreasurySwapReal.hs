{- |
Module      : Fixtures.TxGraph.S11_AmaruTreasurySwapReal
...
on-chain tx shape; 'assertShape' only counts entries, so individual coin
...
Values mirror the on-chain shape but are illustrative only.
