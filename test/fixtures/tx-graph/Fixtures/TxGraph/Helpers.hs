...
'assertShape' skips the @rules.yaml@ on-disk presence
...
a real fixture directory; the directory @test\/fixtures\/tx-graph\/smoke\/@
...
@test/fixtures/tx-graph/@, e.g. @StoryId "02-alice-bob-ada"@.
...
data FixturePaths = FixturePaths
    ...
    --     @test\/fixtures\/tx-graph\/02-alice-bob-ada@.
...
sits under @test\/fixtures\/tx-graph\/<slug>@ and the three
file fields name the per-fixture artifacts (@rules.yaml@ and
...
@specs\/033-tx-graph-harness\/contracts\/goldens-suite.md@ for
the exact contract.
...
this stub so the tx-graph goldens cover the datum-bearing
...
* @fpBlueprint@-or-@esBlueprintRef@ — when 'Just', the referenced file
  must exist on disk,
* @rules.yaml@ — must exist on disk; skipped for 'StoryId' @"smoke"@
  (see module header),
