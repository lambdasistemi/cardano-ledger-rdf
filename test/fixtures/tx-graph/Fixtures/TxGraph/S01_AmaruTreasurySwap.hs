{- |
Module      : Fixtures.TxGraph.S01_AmaruTreasurySwap
...
feature of the tx-graph pipeline at once. Thirty-three
...
wallet. This fixture is the cross-leaf identity reproducer for #43:
the same @recipient@ script hash appears at @body.outputs[0].address@
(as the treasury output) and at every
@body.inputs[*].datum.SwapOrder.recipient@. The emitter must resolve both
to the same @amaru-treasury.network_compliance@ entity label.
...
the @amaru.swap.v2@ witness — those rich graph distinctions (entity
cross-leaf identity, blueprint decode, USDM asset entity) are exercised
by the tx-graph goldens, not here. Per the deferred script-witness policy from S2,
...
network wallet. @assertShape@ only counts, so the USDM amount and the
per-input swap-order coin amounts are not modelled here.
-}
tx :: ConwayTx
tx = mkTx . TxBuilder $ do
    ...
    -- exercised by the tx-graph goldens; the change
